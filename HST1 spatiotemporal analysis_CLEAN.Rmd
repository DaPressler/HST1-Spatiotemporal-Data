...
# QC
Setting extremely implausible values to missing
...
data %>% filter(StepTime_P > 2) %>% select(id, test_session, StepTime_P, StepTime_NP, all_of(outcomes))
data %>% filter(StepTime_P < 0.5) %>% select(id, test_session, StepTime_P, StepTime_NP, all_of(outcomes))
...
#ggsave("Figures/Fig 2.jpg", width=8.5, height=11, device="jpeg")
#ggsave("Figures/Fig 2.pdf", width=8.5, height=11, device=cairo_pdf)
```
...
#ggsave("Figures/Fig 1.jpeg", width=7.5, height=10, device = "jpeg")
#ggsave("Figures/Fig 1.pdf", width=7.5, height=10, device=cairo_pdf)
```
...
```{r}
#Counting partcipants for demographics table
...
```{r}
#Recreating dataset without participant 53 for analysis and table creation
...
#filter out baseline to get separate dataset
Sym_Category.pre = filter(AbsSym_data, test_session == "Baseline") %>%
  select(id, Sym_Category)
#join datasets together with neew column with baseline values in each column
data_Absym=left_join(AbsSym_data, Sym_Category.pre, by="id")
...
#ggsave("Figures/Fig. Curiosity.pdf", width=7.5, height=3, device=cairo_pdf)
```
